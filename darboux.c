...
float *init_W(const mnt *restrict m)
{
  ...
  float *restrict W;
  CHECK((W = calloc(1, ncols * nrows * sizeof(float))) != NULL);
...
mnt *darboux(const mnt *restrict m)
{
   ...
   #pragma omp parallel sections
   {
     #pragma omp section
     {
      for(size_t i = 1; i<size; i++)
        disps[i] = disps[i-1] + ncols;
     }
    #pragma omp section
     {
      for(size_t i = 0; i<size-1; i++)
        counts[i] = ncols*(nrows / size);
      counts[size-1] = ncols*(nrows / size) + remains;
     }
    #pragma omp section
     {
        if(rank == size-1){
          CHECK((Wprec = calloc(1, ((ncols*(nrows / size) + remains)) * sizeof(float))) != NULL);
        }
        else{
          CHECK((Wprec = calloc(1, (ncols*nrows / size) * sizeof(float))) != NULL);
        }
        CHECK((W = calloc(1, ncols * nrows * sizeof(float))) != NULL);
     }
   }
  int receive = (rank==size-1)? ncols*(nrows/size) + remains:ncols*(nrows/size);
  
  if (MPI_Scatterv(W, counts, disps, MPI_FLOAT,W,receive,MPI_FLOAT,0,MPI_COMM_WORLD) != MPI_SUCCESS){
    perror("darboux.c Scatter W error");
    exit(1);
  }
  ...
  while(modif)
  {
    ...
    else if (rank == size - 1)
    {
      ...
      MPI_Ssend(&Wprec[ncols], ncols, MPI_FLOAT, rank - 1, 200, MPI_COMM_WORLD);
    }
    else
    {
      ...
      //send last to next
      MPI_Ssend(&Wprec[(nrows - 2) * ncols], ncols, MPI_FLOAT, rank + 1, 200, MPI_COMM_WORLD);
  ...
  MPI_Gather(&Wprec[rank*(nrows-1)/size+1], (nrows-1)*ncols/size, MPI_FLOAT, &W[rank*(nrows-1)/size+1], (nrows-1)*ncols/size, MPI_FLOAT, 0, MPI_COMM_WORLD);
...
/*---------------FONCTION UTILISÉE POUR LES TESTS DE SPEEDUP-----------------*/
/*****************************************************************************/
/*               Fonction de calcul principale DE RÉFÉRENCE                  */
/*****************************************************************************/
mnt *darboux_ref(const mnt *restrict m)
{
  const int ncols = m->ncols, nrows = m->nrows;
