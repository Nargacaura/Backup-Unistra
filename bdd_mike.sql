...
/*drop*/
/*supression des trigger*/

--drop trigger ONLINE_USERS_date_mod_inactif;
...
CREATE table ONLINE_USERS(
    ...
    /*constraint Activite_ONLINE_USERS check ( (bool_actif=1) or (bool_actif=0))*/
    /*on en a pas besoin, si on laisse le trigger changer bool_actif et le defaul*/
);
/*faire un trigger pour ONLINE_USERS check date_modification*/
...
loop
    ...
    then
        ...
        DBMS_OUTPUT.put_line(row_ONLINE_USERS.nom ||' devenu inactif' );
         ...
         then UPDATE ONLINE_USERS SET bool_actif=1 WHERE idONLINE_USERS=row_ONLINE_USERS.idONLINE_USERS;
...
-- fusion de plusieurs agendas, on va prendre les n premiers agendas et on les fusionne en un
-- FINISHED
...
create or replace trigger no_superposition before update on AGENDAS_ACTIVITE
    for each row
    declare
    CURSOR activite_cur is (select * 
                            from activite inner join AGENDAS_ACTIVITE where 
    begin
